...
def custom_gelu(features):
    with tf.name_scope("Gelu", values=[features]):
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self.shift_size > 0:
            ...
            attn_mask = tf.where(attn_mask != 0, -100.0 * tf.ones_like(attn_mask, dtype=tf.float64),
                                 tf.cast(attn_mask, tf.float64))
            attn_mask = tf.where(attn_mask == 0, tf.zeros_like(attn_mask, dtype=tf.float64),
                                 tf.cast(attn_mask, tf.float64))
